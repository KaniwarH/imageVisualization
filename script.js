        ...
        if (zo[i].childreen[0].markers.length > 20 || zo[i].childreen[1].markers.length > 20 ||
            ...
            if (zo[i].markers.length > 20 && zo[i].markers.length <= 40) {
                ...
                img.onload = function () {
                    ...
                    var icon = L.icon({
                        iconUrl: `6/${zo[i].represent.properties.path}`,//`6/${zo[i].represent.properties.path}`, 
                        iconSize: [newWidth, newHeight],
                    });
                    let newMarker = new L.marker(zo[i].coordinates, { icon: icon }); //.addTo(map);
                    newMarker.bindPopup(`
                    <h3 id = pup> Name: ${zo[i].represent.properties.name}</h3>
                    <h3 id = pup>Date: ${zo[i].represent.properties.date}</h3>
                    <h3 id = pup>Cluster size: ${zo[i].markers.length}</h3>
                    For more information click<a href = ${zo[i].represent.
                            properties.link} target="_blank"> here</a>`
                    ).openPopup();
                    newMarker.addTo(marker);
                    //new L.marker(zo[i].coordinates, { icon: myDivIcon1 }).addTo(marker)
                }
                img.src = `6/${zo[i].represent.properties.path}`;//`6/${zo[i].represent.properties.path}`;
            ...
            for (let child of zo[i].childreen) {
                ...
                if (child.markers.length > 1 && child.markers.length <= 10) {
                    ...
                    img.onload = function () {
                        ...
                        let icon = L.icon({
                            iconUrl: `6/${child.represent.properties.path}`,//`6/${child.represent.properties.path}`,
                            iconSize: [newWidth, newHeight],
                        });
                        let newMarker = new L.marker(child.coordinates, { icon: icon }); //.addTo(map);
                        newMarker.bindPopup(`
                        <h3 id = pup>Name: ${child.represent.properties.name}</h3>
                        <h3 id = pup>Date: ${child.represent.properties.date}</h3>
                        <h3 id = pup>Cluster size: ${child.markers.length}</h3>
                        For more information click<a href = ${child.represent.
                                properties.link} target="_blank"> here</a>`
                        ).openPopup();
                        newMarker.addTo(marker)
                        //new L.marker(zo[i].coordinates, { icon: myDivIcon1 }).addTo(marker);
                    ...
                    img.src = `6/${child.represent.properties.path}`;//`6/${child.represent.properties.path}`;
                }
